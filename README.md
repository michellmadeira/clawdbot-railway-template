...
**Arquitetura repo vs drive:** Este repositório é **código-fonte** (Docker, servidor, VPS). As **skills, agentes e docs que o agente lê** ficam e devem ser editados no **drive persistente** (workspace OpenClaw conectado via SSH/WebDAV, ex.: `Z:\workspace` ou `/data/workspace`). Ver **[docs/ARQUITETURA-REPO-VS-DRIVE.md](docs/ARQUITETURA-REPO-VS-DRIVE.md)**.
